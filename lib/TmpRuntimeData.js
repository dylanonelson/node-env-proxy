...
var RUNTIME_CONFIG_FILE_PATH = path.resolve(
...
TmpRuntimeData.prototype.cleanUpPid = function() {
  try { fs.unlinkSync(PID_FILE_PATH); }
  catch (e) {}
}

TmpRuntimeData.prototype.cleanUpRuntimeConfig = function() {
  try { fs.unlinkSync(RUNTIME_CONFIG_FILE_PATH); }
  catch (e) {}
}
...
TmpRuntimeData.prototype.getRuntimeConfig = function() {
  return JSON.parse(
    fs.readFileSync(RUNTIME_CONFIG_FILE_PATH).toString()
  );
}

TmpRuntimeData.prototype.writeRuntimeConfig = function(config) {
  try { fs.writeFileSync(RUNTIME_CONFIG_FILE_PATH, JSON.stringify(config)); }
  catch (e) { throw e; }
